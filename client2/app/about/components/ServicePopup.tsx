...
interface ChangeServicePopupProps {
    onClose: () => void;
...
const ServicePopup: React.FC<ChangeServicePopupProps> = ({ onClose, onSelectService }) => {
    const [searchResults, setSearchResults] = useState<Service[]>([]);
    const { register, handleSubmit, formState: { errors } } = useForm<SearchFormInputs>({
        resolver: zodResolver(searchSchema),
    });

    const searchServices: SubmitHandler<SearchFormInputs> = async (data) => {
        ...
        try {
            const response = await fetch(`http://localhost:4000/api/v1/services/search?${queryParams}`, {
                method: 'GET',
                headers: {
            ...
            const results: Service[] = await response.json();
    ...
    const handleSelect = (service: Service) => {
        onSelectService(service); // Pass the selected service back to the parent component
    ...
    return (
        <div className="absolute top-0 left-0 right-0 bottom-0 bg-black bg-opacity-50 flex justify-center items-center z-10">
            <div className="bg-white p-4 rounded-lg shadow-lg w-full max-w-lg">
                <h2 className="text-lg font-bold mb-4">Search Services</h2>
                <form onSubmit={handleSubmit(searchServices)} className="space-y-4">
                    <input
                        {...register('name')}
                ...
                <div className="max-h-60 overflow-auto">
                    {searchResults?.map((service) => (
                        ...
                        >
                            {`${service.name} - ${service.location} - $${service.price} - ${service.isActive ? 'Active' : 'Inactive'}`}
                        </div>
                    ))}
                </div>
